...
#define VERSION "2026"
#define AUTHOR "Abdoulaye Diallo"
#define EMAIL "adiallo06102@gmail.com"
...
int main(int argc, char *argv[]) {
    if (argc != 2) {
        fprintf(stderr, "./bm.out: Invalid number of arguments\n"); /*si le nombre d'arguments passés en ligne de commande est incorrect*/
        return 1;
    }

    const char *filename = argv[1];     /*récupère le nom du fichier xml à lire*/
    ...
    while (fgets(command, sizeof(command), stdin) != NULL) {
        ...
        if (strlen(command) > MAX_COMMAND_LENGTH) {
            printf("./bm.out: Too many characters for the command\n"); /*vérifie si le nombre de caractères max pour les commandes n'est pas dépassé*/
